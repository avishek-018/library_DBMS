...
try {
    ...
    $stmt = $pdo->prepare('UPDATE Reservation SET Status = "returned" WHERE ID = ? AND Status = "checked_out"');
    $stmt->execute([$reservation_id]);

    if ($stmt->rowCount() === 0) {
        throw new Exception('Reservation not found or not checked out');
    }
    ...
    $pdo->commit();
    header('Location: dashboard.php?success=Reservation+returned');
    exit;
} catch (Exception $e) {
    $pdo->rollBack();
    header('Location: dashboard.php?error=Failed+to+return+reservation:+' . urlencode($e->getMessage()));
    exit;
}
?>